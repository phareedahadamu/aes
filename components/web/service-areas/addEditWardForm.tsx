...
import { AddEditWardSchema } from "@/lib/dal/service-areas/schema.serviceAreas";
import { addWard, editWard } from "@/lib/dal/service-areas/serviceAreasActions";

export default function AddEditWardForm({
  action,
  ward,
  closeModal,
}:
  ...
  const [addResponse, addAction, isPendingAdd] = useActionState(addWard, null);
  ...
  const {
    ...
    resolver: zodResolver(AddEditWardSchema),
    defaultValues: {
      code: ward ? ward.code : "",
      name: ward ? ward.name : "",
    },
  ...
  const onSubmit: SubmitHandler<z.infer<typeof AddEditWardSchema>> = (data) => {
