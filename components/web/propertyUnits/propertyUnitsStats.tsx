...
import { getPropertyUnitsStats } from "@/lib/dal/property-units/getPropertyUnits";
import { Eye, EyeOff, Blocks, Loader2 } from "lucide-react";
import { ReactNode, useEffect } from "react";
import { toast } from "sonner";
import { PROPERTY_UNITS_METRICS_QKEY } from "@/lib/constants/general";
export default function PropertyUnitsStats() {
  ...
  useEffect(() => {
    if (error || (data && !data.success)) {
      toast.error(
        "Something went wrong while retrieving property units stats. Try again later",
      );
    }
  }, [data, error]);
  return (
